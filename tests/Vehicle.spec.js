...
const chatbotData = require('../data/vehicle.json');
...
test('Validate Chatbot Responses', async ({ page }) => {
    ...
    for (const item of chatbotData.testCases) {
        ...
        const matchedKeywords = item.expectedKeywords.filter(
            keyword =>
                response
                    .toLowerCase()
                    .includes(keyword.toLowerCase())
        ...
        const status = missingKeywords.length > 0 ? "FAIL" : "PASS";
    ...
    fs.writeFileSync(
        "./test-results/vehicle-report.json",
    ...
    if (errors.length > 0) {
        ...
        // throw new Error(`${errors.length} chatbot test cases failed.`);
    }
