...
const chatbotData = require('../data/response.json');
...
test('Validate Chatbot Responses', async ({ page }) => {
    ...
    for (const item of chatbotData) {
        ...
        await chatbot.sendMessage(item.message);
        ...
        if (matchedKeywords.length > 0) {
            ...
            console.log(`${item.testCaseId} PASSED`);

        } else {

            console.log(`${item.testCaseId} FAILED`);
    ...
    console.log('\nresponse.json generated successfully');

    if (errors.length > 0) {

        console.log(`Found ${errors.length} failed test case(s).`);
